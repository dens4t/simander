...
export async function handleSubkegitan(request, env, ctx) {
  ...
  if (method === 'GET' && (path === '' || path === '/')) {
    ...
    try {
      ...
      if (search) {
        whereClause += ' AND (subkegiatan LIKE ? OR ppk LIKE ?)'
      ...
      const dataQuery = await env.order_2025_db.prepare(
        `SELECT * FROM subkegiatan WHERE ${whereClause} ORDER BY subkegiatan ASC LIMIT ? OFFSET ?`
  ...
  if (method === 'GET' && /^\/\d+$/.test(path)) {
    ...
    try {
      const result = await env.order_2025_db.prepare(
        'SELECT * FROM subkegiatan WHERE id = ?'
      ).bind(subkegiatanId).first()

      if (!result) {
        return new Response(JSON.stringify({ message: 'Subkegitan tidak ditemukan' }), {
          status: 404,
          headers: { 'Content-Type': 'application/json' }
        })
      }
  ...
  if (method === 'POST' && (path === '' || path === '/')) {
    ...
    try {
      const result = await env.order_2025_db.prepare(
        `INSERT INTO subkegiatan (subkegiatan, ppk, status, created_at, updated_at) 
         VALUES (?, ?, ?, ?, ?)`
      ).bind(
        data.subkegiatan,
        data.ppk || '',
  ...
  if (method === 'PUT' && /^\/\d+$/.test(path)) {
    ...
    try {
      ...
      await env.order_2025_db.prepare(
        `UPDATE subkegiatan SET subkegiatan = ?, ppk = ?, status = ?, updated_at = ? WHERE id = ?`
      ).bind(
      ...
      return new Response(JSON.stringify({ message: 'Subkegitan berhasil diupdate' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' }
      })
    } catch (error) {
  ...
  if (method === 'DELETE' && /^\/\d+$/.test(path)) {
    ...
    try {
      ...
      return new Response(JSON.stringify({ message: 'Subkegitan berhasil dihapus' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' }
      })
    } catch (error) {
